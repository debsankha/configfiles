...
let b:template_1 = '~/.vim/ftplugin/tex/latex'
let b:template_2 = '~/.vim/min-template'
let b:template_3 = '~/.vim/sci-template'
let b:template_4 = '~/.vim/min-latex'
...
"let b:latex_command = "!pdflatex -interaction nonstopmode -synctex=1 %"
...
function! s:TexInsertTabWrapper(direction) 
	...
	let ln = ln - 1
    endwhile
    if getline(ln) =~ 'begin{\(eq\|dmath\|arr\|align\|mult\)\|\\\['
	let math = 1
    endif
...
inoremap <buffer><silent> <F2> \begin{figure}<CR>\caption{}<CR>\begin{center}<CR>\includegraphics[width=0.9\columnwidth]{}<CR>\end{center}<CR>\end{figure}<Esc>2k$i
...
function! s:FFour(var)
	...
	return "\\begin{equation}\<CR>\\label{}\<CR>\\begin{aligned}\<CR>\<CR>\\end{aligned}\<CR>\\end{equation}\<Up>\<UP>"
    endif
endfunction
